...
Live capture monitor. This is how auto-recording works — OCR scans the game feed to detect deployment (start recording) and lobby return (stop recording). Shows the game window with scan regions visualized, engine status, recording state, and the full processing pipeline — Phase 0 (queue), Phase 1 (frames + stats), Phase 2 (narrative + clips). Processing queue shows each recording with pipeline progress and status.
