...
pub enum VideoEncoderError {
    ...
    #[error("Failed to snapshot a WGC frame for the encoder: {0}")]
    FrameSnapshotError(#[from] crate::frame::Error),
    #[error("Encoder snapshot queue is full")]
    FrameBackpressure,
}
...
type SnapshotRelease = Arc<(Mutex<VecDeque<usize>>, Condvar)>;
type VideoFrameReceiver =
    Arc<Mutex<mpsc::Receiver<Option<(VideoEncoderSource, TimeSpan)>>>>;
type AudioFrameReceiver =
    Arc<Mutex<mpsc::Receiver<Option<(AudioEncoderSource, TimeSpan)>>>>;

pub enum VideoEncoderSource {
    DirectX { surface: SendDirectX<IDirect3DSurface>, slot: usize, release: SnapshotRelease },
...
impl VideoSettingsBuilder {
    ...
    /// Describe the incoming frame independently from the encoded output.
    /// Media Transcoder can then scale the full Direct3D surface.
    pub const fn input_size(mut self, width: u32, height: u32) -> Self {
    ...
    fn build(self) -> Result<(VideoEncodingProperties, bool, (u32, u32)), VideoEncoderError> {
        ...
        Ok((properties, self.disabled, (self.input_width, self.input_height)))
    }
}
...
impl VideoEncoder {
    /// Copy the current WGC texture into a retained GPU texture. WGC's frame
    /// pool reuses its own surface as soon as the capture callback returns,
    /// while Media Foundation consumes samples asynchronously. Passing the WGC
    /// surface directly therefore lets later captures overwrite frames that
    /// have not been encoded yet (visible as multi-second freezes/jumps).
    ///
    /// A small recycled GPU texture pool gives each in-flight sample stable
    /// pixels without a CPU readback. Slots return through MediaStreamSample's
    /// Processed event once the hardware encoder is done with them.
    fn snapshot_frame(
        &mut self,
        frame: &Frame,
    ) -> Result<(usize, IDirect3DSurface), VideoEncoderError> {
        // Never wait inside WGC's FrameArrived callback. Blocking that callback
        // prevents the frame pool from returning new game frames and
        // produces a nominal 60 FPS file containing only one real frame every
        // few seconds. Eight retained 4K textures bound worst-case VRAM while
        // giving Media Foundation enough asynchronous pipeline headroom.
        const SNAPSHOT_POOL_LIMIT: usize = 8;

        let mut available = self.snapshot_release.0.lock();
        let slot = available.pop_front();
        drop(available);

        let slot = if let Some(slot) = slot {
            frame.copy_to_encoder_snapshot(&self.snapshot_slots[slot].texture);
            slot
        } else if self.snapshot_slots.len() < SNAPSHOT_POOL_LIMIT {
            let (texture, surface) = frame.create_encoder_snapshot()?;
            let slot = self.snapshot_slots.len();
            self.snapshot_slots.push(SnapshotSlot { texture, surface });
            slot
        } else {
            return Err(VideoEncoderError::FrameBackpressure);
        };

        Ok((slot, self.snapshot_slots[slot].surface.clone()))
    }

    /// Feed Media Foundation from Windows' worker pool. `SampleRequested` is
    /// an event callback; blocking it on `Receiver::recv` can stall the whole
    /// media pipeline and, in turn, the WGC callback that submits frames.
    /// A deferral lets the event return immediately while a worker waits for
    /// the next GPU surface.
    fn attach_sample_requested_handler(
        media_stream_source: &MediaStreamSource,
        is_video_disabled: bool,
        is_audio_disabled: bool,
        ...
        >::new(move |_, sample_requested| {
            ...
            if is_audio {
                    ...
                    ThreadPool::RunWithPriorityAndOptionsAsync(
                        &WorkItemHandler::new(move |_| {
                            match audio_receiver.lock().recv() {
                                ...
                                Ok(None) | Err(_) => request_clone.SetSample(None)?,
                            }
                ...
                ThreadPool::RunWithPriorityAndOptionsAsync(
                    &WorkItemHandler::new(move |_| {
                        match frame_receiver.lock().recv() {
                            Ok(Some((source, timestamp))) => {
                                let (sample, snapshot_to_release) = match source {
                                ...
                                // The original encoder synchronously held the
                                // WGC callback until SetSample completed, then
                                // returned that WGC surface to its frame pool.
                                // We preserve the same lifetime boundary for
                                // our copied texture, but release only this
                                // private snapshot slot. This keeps capture
                                // asynchronous without relying on Processed,
                                // which MediaTranscoder may defer until drain.
                                let set_result = request_clone.SetSample(&sample);
                                if let Some((slot, release)) = snapshot_to_release {
                                    let (lock, cvar) = &*release;
                                    lock.lock().push_back(slot);
                                    cvar.notify_one();
                                }
                                set_result?;
                            }
    ...
    pub fn new<P: AsRef<Path>>(
        ...
        let (video_encoding_properties, is_video_disabled, (input_width, input_height)) =
            video_settings.build()?;
        ...
        let sample_requested = Self::attach_sample_requested_handler(
            &media_stream_source,
            is_video_disabled,
            is_audio_disabled,
            Arc::new(Mutex::new(frame_receiver)),
            Arc::new(Mutex::new(audio_receiver)),
            frame_notify.clone(),
            audio_notify.clone(),
        )?;
        ...
        Ok(Self {
            ...
            snapshot_release: Arc::new((Mutex::new(VecDeque::new()), Condvar::new())),
            is_video_disabled,
            is_audio_disabled,
        })
    }
    ...
    pub fn send_frame(&mut self, frame: &mut Frame) -> Result<(), VideoEncoderError> {
        ...
        let (slot, surface) = self.snapshot_frame(frame)?;
        let payload = Some((
            VideoEncoderSource::DirectX {
                surface: SendDirectX::new(surface),
                slot,
                release: self.snapshot_release.clone(),
            },
            timestamp,
        ));
        if let Err(error) = self.frame_sender.send(payload) {
            // Media Foundation has already stopped asking for samples. Return
            // the retained GPU slot before surfacing the channel error, or
            // repeated start/stop failures would permanently exhaust the pool.
            if let Some((
                VideoEncoderSource::DirectX { slot, release, .. },
                _,
            )) = &error.0
            {
                let (lock, cvar) = &**release;
