...
impl<'a> Frame<'a> {
    ...
    /// Allocate a GPU-resident texture that can outlive this WGC callback.
    pub fn create_encoder_snapshot(&self) -> Result<(ID3D11Texture2D, IDirect3DSurface), Error> {
        let mut texture_desc = D3D11_TEXTURE2D_DESC::default();
        unsafe {
            self.frame_texture.GetDesc(&mut texture_desc);
        }
        ...
        let texture = texture.expect("CreateTexture2D returned no encoder snapshot");
        self.copy_to_encoder_snapshot(&texture);
    ...
    /// Refresh a retained encoder texture with the current WGC frame on-GPU.
    pub fn copy_to_encoder_snapshot(&self, texture: &ID3D11Texture2D) {
        unsafe {
